"""
Biometric Authentication Agent (login flow)
Uses the Claude SDK agentic loop to orchestrate 4 tools in strict sequence:
  ...
  3. verify_identity        — cosine similarity vs stored face template
...
Instrumented with Langfuse (LLM tracing) and Prometheus (metrics).
"""
import json
import time
import uuid
...
from tools.verify_identity import TOOL_SPEC as VERIFY_SPEC

log = structlog.get_logger()

_SYSTEM_PROMPT = """You are a biometric authentication agent for a regulated Singapore banking application.

Your job is to authenticate a RETURNING user via facial biometrics by calling four tools IN STRICT ORDER:

STEP 1 — Call validate_face_capture(username)
  - If valid=false: respond with authentication_failed and reason. STOP.
  - If valid=true: proceed to step 2.

STEP 2 — Call detect_liveness(username)
  - If passed=false: respond with authentication_failed and reason. STOP.
  - If passed=true: proceed to step 3.

STEP 3 — Call verify_identity(username)
  - If matched=false: respond with authentication_failed and reason. STOP.
  - If matched=true: proceed to step 4.

STEP 4 — Call issue_keycloak_token(username, liveness_score, deepfake_score)
  - If success=false: respond with authentication_failed and reason.
  - If success=true: respond with authentication_success and the access_token.

RULES:
- Never skip a step. Never call step 4 before steps 2 and 3 both pass.
- Never fabricate scores or tokens.
- If any tool returns an error, treat it as authentication_failed.
...
# Slim tool specs — image_b64 and embedding removed so they never enter Claude's context
_FACE_SPEC_SLIM = {
    ...
    "input_schema": {
        "type": "object",
        "properties": {
            "username": {"type": "string", "description": "Username attempting authentication."},
        },
        "required": ["username"],
    },
}
...
_VERIFY_SPEC_SLIM = {
    ...
    "input_schema": {
        "type": "object",
        "properties": {
            "username": {"type": "string", "description": "Username to verify."},
        },
        "required": ["username"],
    },
}

_ALL_TOOLS = [_FACE_SPEC_SLIM, _LIVENESS_SPEC_SLIM, _VERIFY_SPEC_SLIM, TOKEN_SPEC]


def run_auth(image_b64: str, username: str, request_id: str | None = None) -> dict[str, Any]:
    ...
    messages = [
        {
            "role": "user",
            "content": f"Authenticate user '{username}'. Request ID: {request_id}",
        }
    ]
    ...
    # Per-request embedding cache: populated by detect_liveness, consumed by verify_identity
    ...
    while True:
            ...
            for block in response.content:
                if block.type == "text":
                    ...
                    if result is not None:
                        break
                    log.warning("agent_non_json_response", text=block.text[:300])
        ...
        if response.stop_reason == "tool_use":
            ...
            for block in response.content:
                ...
                if handler is None:
                    ...
                    try:
                        if tool_name in ("validate_face_capture", "detect_liveness"):
                            ...
                            # Stash embedding for verify_identity
                ...
                # Strip raw embedding from tool result sent back to Claude (privacy + token savings)
...
def _finalize(lf_trace, tracer, result: dict, total_in: int, total_out: int) -> None:
    lf_trace.update(
        ...
        level="DEFAULT" if result.get("status") == "authentication_success" else "WARNING",
    )
    tracer.flush()
